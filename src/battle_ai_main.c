...
static u32 AI_CalcMoveEffectScore(u32 battlerAtk, u32 battlerDef, u32 move)
{
    ...
    case EFFECT_PARTING_SHOT:
        ...
        else //Double Battle
        {
            ...
            if (aiData->abilities[battlerAtk] == ABILITY_INTIMIDATE && PartyHasMoveCategory(battlerDef, DAMAGE_CATEGORY_PHYSICAL))
                ADJUST_SCORE(7);

            if (aiData->abilities[battlerAtk] == ABILITY_PSYCH_OUT && PartyHasMoveCategory(battlerDef, DAMAGE_CATEGORY_SPECIAL))
                ADJUST_SCORE(7);
        }
        break;
    case EFFECT_BATON_PASS:
