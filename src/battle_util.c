...
bool32 CanBeConfused(u8 battlerId)
{
    if (GetBattlerAbility(battlerId) == ABILITY_OWN_TEMPO
      || GetBattlerAbility(battlerId) == ABILITY_FREE_SPIRIT
...
static bool32 IsBattlerGrounded2(u8 battlerId, bool32 considerInverse)
{
    ...
    if (GetBattlerHoldEffect(battlerId, TRUE) == HOLD_EFFECT_AIR_BALLOON)
        return FALSE;
    if (GetBattlerAbility(battlerId) == ABILITY_LEVITATE || GetBattlerAbility(battlerId) == ABILITY_SKY_SONATA)
        return FALSE;
...
static u32 CalcAttackStat(u16 move, u8 battlerAtk, u8 battlerDef, u8 moveType, bool32 isCrit, bool32 updateFlags)
{
    ...
    case ABILITY_PLUS:
        ...
        break;
    case ABILITY_MINUS:
    if (IS_MOVE_SPECIAL(move) && IsBattlerAlive(BATTLE_PARTNER(battlerDef)))
    {
    ...
    case ABILITY_PLUS:
        ...
        break;
    case ABILITY_MINUS:
        if (IS_MOVE_SPECIAL(move) && IsBattlerAlive(BATTLE_PARTNER(battlerDef)) && GetBattlerAbility(BATTLE_PARTNER(battlerDef)) == ABILITY_PLUS)
            MulModifier(&modifier, UQ_4_12(1.5));
        break;
    #endif
    case ABILITY_FLOWER_GIFT:
...
static void MulByTypeEffectiveness(u16 *modifier, u16 move, u8 moveType, u8 battlerDef, u8 defType, u8 battlerAtk, bool32 recordAbilities)
{
    ...
    if (GetBattlerAbility(battlerAtk) == ABILITY_NORMALIZE && (defType == TYPE_ROCK || defType == TYPE_GHOST || defType == TYPE_STEEL))
        mod = UQ_4_12(1.0);
